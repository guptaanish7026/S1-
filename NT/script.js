document.addEventListener("DOMContentLoaded", () => {
    ...
    const ICONS = { study: `<svg viewBox="0 0 24 24" width="18" height="18" stroke="currentColor" stroke-width="2" fill="none"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>` };

    const initApp = () => {
        setupTheme();
        if (!user) DOM.modal.classList.add("active");
    ...
    const updateProfileUI = () => {
        ...
        const avatarHTML = user.profileImage 
            ? `<img src="${user.profileImage}" alt="User" class="avatar">`
    ...
    DOM.profileBtn.addEventListener("click", (e) => { e.stopPropagation(); DOM.profilePanel.classList.toggle("active"); });
    document.addEventListener("click", (e) => { if (!DOM.profilePanel.contains(e.target) && e.target !== DOM.profileBtn) DOM.profilePanel.classList.remove("active"); });

    // FIXED: Never wipes Device ID on clear
    DOM.clearDataBtn.addEventListener("click", () => {
        if (confirm("Reset application? Your Profile and Favorites will be cleared.")) {
            const preservedDeviceId = localStorage.getItem("nt_device_id");
            localStorage.clear();
            if (preservedDeviceId) localStorage.setItem("nt_device_id", preservedDeviceId);
            location.reload();
        }
    });

    // Uses the Secure CORS Proxy method we created in verification.js
    const fetchBatches = async () => {
        try {
            const data = await window.Verification.secureFetch("https://study-one-access.vercel.app/api/batches");
            ...
            DOM.loadingState.classList.add("hidden");
            DOM.statusMsg.innerHTML = "Network Blocked. Unable to load batches.";
    ...
    const renderBatches = () => {
        ...
        if (filtered.length === 0) {
            DOM.statusMsg.textContent = activeTab === "favorites" && !searchQuery ? "No favorites saved yet. Add some ❤️" : "No results found.";
            DOM.statusMsg.classList.remove("hidden");
            return;
        }
        ...
        filtered.forEach(batch => {
            ...
            card.innerHTML = `
                <div class="thumb-wrapper">
                    <img src="${batch.thumbnail}" alt="${batch.title}" class="batch-thumb" loading="lazy">
    ...
    const bindEvents = () => {
        ...
        DOM.searchInput.addEventListener("input", (e) => { searchQuery = e.target.value; renderBatches(); });
